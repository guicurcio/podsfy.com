...
const generateGoodTitleForReviews =
 (title) => {
  let newTitle =
   title
    .split(' ')
...
const getPodcastsToStaticallyGenerate =
 async () => {
...
export async function generateStaticParams() {
 const podcasts =
  await getPodcastsToStaticallyGenerate();
 return podcasts.map(
  (podcast) => ({
   pod: podcast.slug,
  })
    ...
    <div className="max-w-[1200px] mx-auto rounded-lg z-20  bg-[#0D0E12] bg-opacity-90">
     <div className="grid grid-flow-col  gap-12  justify-start backdrop-brightness-[50%] w-full p-8 rounded-xl">
      <div className=" rounded-md flex flex-col justify-start">
       ...
       <PodBehind
        genre={
         staticPodcastData.genre
        }
        rating={
         staticPodcastData.rating
        }
        tags={
         staticPodcastData.tags
        }
        podcastTitle={
         staticPodcastData.title
        }
      ...
      <div className="justify-start items-start grid grid-flow-row gap-16 h-full w-[650px]">
       <div>
        <PodDetails
         title={
          staticPodcastData.title
       ...
       <div className="justify-start items-start grid grid-flow-row gap-8 h-full w-[650px]">
        {staticPodcastData.episodes.some(
         (
          episode
         ) =>
